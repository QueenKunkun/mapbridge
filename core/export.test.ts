...
describe('core/export', () => {
  ...
  it('rejects unsupported v2 versions', () => {
    expect(() =>
      parsePlacesFile(JSON.stringify({ format: 'mapbridge', version: 3, items: [] })),
    ).toThrow(/不支持的 MapBridge 文件版本/);
  });
  ...
  it('round-trips a v2 document containing a Route', () => {
    ...
    const parsedPlaces = parsePlacesFile(
      serializeItems([migratePlaceToPoi(place), route], 'baidu'),
    );
