...
describe('core/jobs unified items', () => {
  it.each([
    ...
    const job = {
      ...createJob('baidu', 'amap', workflow),
      status,
      updatedAt: '2026-09-09T00:00:00.000Z',
    };
    expect(restorePopupState([job], { mode: 'migrate' })).toMatchObject({
      kind: 'active',
      mode: workflow,
      step,
      job,
  ...
  it.each(['done', 'failed', 'cancelled', 'draft'] as const)(
    'does not restore %s jobs',
    (status) => {
      const job = { ...createJob('baidu', 'amap'), status, updatedAt: '2026-09-09T00:00:00.000Z' };
      expect(restorePopupState([job], { mode: 'export' })).toEqual({
        kind: 'idle',
        mode: 'export',
      });
    },
  ...
  it('never restores an export extraction as migration preview', () => {
    const job = {
      ...createJob('amap', 'amap', 'export'),
      status: 'preview' as const,
      updatedAt: '2026-09-09T00:00:00.000Z',
    };
    expect(restorePopupState([job], { mode: 'migrate' })).toEqual({
      kind: 'idle',
      mode: 'migrate',
    });
  });
  ...
  it('restores only the explicitly active job when an active id is available', () => {
    ...
    const newer = {
      ...createJob('amap', 'baidu'),
      status: 'preview' as const,
      updatedAt: '2026-09-09T01:00:00.000Z',
    };
    expect(restorePopupState([older, newer], { mode: 'migrate' }, older.id)).toMatchObject({
      kind: 'active',
      job: older,
    });
  });
  ...
  it("does not restore another tab's task when the current tab has no active task", () => {
    ...
    expect(restorePopupState([job], { mode: 'migrate' }, undefined, true)).toEqual({
      kind: 'idle',
      mode: 'migrate',
    });
  });
  ...
  it('updates only the current POI match row as matching and completes rows individually', () => {
    const job = createJob('baidu', 'amap');
    const first = applyAmapPoiMatchProgress(job, {
      currentPlaceId: 'poi-1',
      processed: 0,
      total: 2,
    });
  ...
  it('persists Route items while keeping POI places as the import view', () => {
    const job = applyExtractionItems(
      createJob('baidu', 'amap'),
      [route, { kind: 'poi', ...place, geometry: { type: 'point', point: place.wgs84 } }],
      [place],
      2,
    );
  ...
  it('hydrates legacy jobs without items or warnings', () => {
    ...
    const {
      items: _items,
      warnings: _warnings,
      extractionSkipped: _extractionSkipped,
      rawCount: _rawCount,
      ...legacy
  ...
  it('invalidates identity when a preview name changes', () => {
    ...
    expect(updatePreviewPlace(withIdentity, { address: 'New address' }).identity).toBe(
      'old-identity',
    );
  ...
  it('marks a provider import failure as failed', () => {
    ...
    const failed = finalizeImport(
      job,
      { provider: 'amap', done: false, error: 'network failed' },
      {
        imported: 0,
        skippedDuplicates: 0,
        failed: 1,
        failedItems: [{ placeId: 'poi-1', error: 'network failed' }],
      },
    );
