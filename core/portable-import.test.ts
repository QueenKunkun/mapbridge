...
describe('portable import', () => {
  it('imports a representative external GPX waypoint without MapBridge extensions', () => {
    const source = readFileSync(
      new URL('../test-fixtures/portable/external-waypoints.gpx', import.meta.url),
      'utf8',
    );
  ...
  it('imports a representative external KML Point nested in a Folder', () => {
    const source = readFileSync(
      new URL('../test-fixtures/portable/external-folder.kml', import.meta.url),
      'utf8',
    );
  ...
  it('imports GPX waypoints and preserves MapBridge extensions', () => {
    const result = parsePortableImport(
      `<?xml version="1.0"?><gpx><wpt lat="30" lon="104"><name>Cafe</name><desc>Road</desc><extensions><mapbridge:tags xmlns:mapbridge="https://mapbridge.app/ns/gpx">food;成都</mapbridge:tags><mapbridge:phone xmlns:mapbridge="https://mapbridge.app/ns/gpx">123</mapbridge:phone><mapbridge:identity xmlns:mapbridge="https://mapbridge.app/ns/gpx">cafe|104.00000|30.00000</mapbridge:identity><mapbridge:sourceRecordId xmlns:mapbridge="https://mapbridge.app/ns/gpx">source-1</mapbridge:sourceRecordId></extensions></wpt><rte><name>Route</name><rtept lat="30" lon="104" /></rte></gpx>`,
      'amap',
    );
    expect(result.items).toHaveLength(1);
  ...
  it('imports KML Points and skips LineString routes explicitly', () => {
    const result = parsePortableImport(
      '<kml><Document><Placemark><name>Park</name><description>Green</description><ExtendedData><Data name="identity"><value>park|104.00000|30.00000</value></Data><Data name="sourceRecordId"><value>source-2</value></Data></ExtendedData><Point><coordinates>104,30,0</coordinates></Point></Placemark><Placemark><name>Route</name><LineString><coordinates>104,30,0 105,31,0</coordinates></LineString></Placemark></Document></kml>',
      'baidu',
    );
  ...
  it('rejects XML without supported points', () => {
    expect(() =>
      parsePortableImport('<gpx><rte><name>Only route</name></rte></gpx>', 'amap'),
    ).toThrow(/没有可导入的 GPX waypoint/);
  });
  ...
  it('dispatches JSON and XML through one portable-file entry point', () => {
    ...
    const xml = parsePortableFile(
      '<gpx><wpt lat="30" lon="104"><name>XML POI</name></wpt></gpx>',
      'amap',
    );
  ...
  it('accepts namespace-qualified GPX and KML documents', () => {
    const gpx = parsePortableImport(
      '<gpx version="1.1" xmlns="http://www.topografix.com/GPX/1/1" xmlns:mapbridge="https://mapbridge.app/ns/gpx"><wpt lat="30" lon="104"><name>GPX POI</name><extensions><mapbridge:identity>gpx-id</mapbridge:identity></extensions></wpt></gpx>',
      'amap',
    );
    const kml = parsePortableImport(
      '<kml xmlns="http://www.opengis.net/kml/2.2"><Document><Placemark><name>KML POI</name><Point><coordinates>104,30,0</coordinates></Point></Placemark></Document></kml>',
      'baidu',
    );
