...
function groupExtractionSkips(
  skips: Job['extractionSkipped'],
): { reason: string; items: typeof skips }[] {
...
export function NextImportButton({
  disabled,
  onClick,
}: {
  disabled?: boolean;
  ...
  return (
    <button className="primary" disabled={disabled} onClick={() => void onClick()}>
      下一步：导入 →
    </button>
  );
}
