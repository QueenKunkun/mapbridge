...
export function RouteSummary({
  ...
  route: Extract<Job['items'][number], { kind: 'route' }>;
}) {
      ...
      <ol>
        {route.stops.map((stop) => (
          <li key={`${stop.role}-${stop.point.lng}-${stop.point.lat}`}>
            <span>
              {roleName[stop.role] ?? stop.role}：{stop.name}
            </span>
            <code>
              {stop.point.lng.toFixed(5)}, {stop.point.lat.toFixed(5)}
            </code>
          </li>
        ))}
      </ol>
...
export function PlaceTable({
  ...
  const update = (id: string, patch: Partial<Job['places'][number]>) =>
    onChange(places.map((p) => (p.id === id ? updatePreviewPlace(p, patch) : p)));
  const remove = (id: string) => onChange(places.filter((p) => p.id !== id));
  return (
    <div className={`place-table${canMatchAmap ? ' can-match' : ''}`}>
      <div className="place-toolbar">
        <label className="page-size" title="每页显示数量">
          ...
          <select
            value={pageSize}
            onChange={(e) => {
              setPageSize(Number(e.target.value) as 10 | 30 | 50);
              setPage(1);
            }}
          >
        ...
        <div className="place-search">
          <button
            className={`small icon-button${searchOpen ? ' active' : ''}`}
            aria-label="搜索地点"
            title="搜索地点"
        ...
        {canMatchAmap && (
          <span className="match-action">
            ...
            <PoiHelpButton
              label="尝试批量地址匹配"
              tooltip={
                '可选项。\n1. 无需匹配也可导入。\n2. 匹配目标地图的原生地点并关联之后，信息会更多，位置会更准确。'
              }
            />
          </span>
        )}
      ...
      <div className="table-body">
        {shown.map((place, index) => (
          <PlaceRow
            ...
            resolution={amapPoiResolutions?.[place.id]}
            matching={matchingPlaceIds?.has(place.id) ?? false}
      ...
      <div className="table-foot">
        ...
        {pageCount > 1 && (
          <div className="pagination" aria-label="地点分页">
            <button
  ...
  return (
    <div className="place-row">
      <div className="row">
        ...
        <input
          value={place.address ?? ''}
          title={`地址：${place.address ?? ''}`}
          onChange={(e) => onChange(place.id, { address: e.target.value })}
        />
        {canMatchAmap && (
          <PoiMatchCell
            place={place}
...
function PoiMatchCell({
  ...
  if (matching || match?.status === 'matching')
    return (
      <span className="match-progress" role="status">
        匹配中…
      </span>
    );
  ...
  const selector =
    candidates.length > 0 ? (
      <select
        ...
        aria-label={`为${place.name}选择目标地图 POI`}
        onChange={(event) =>
          onSelect?.(
            place.id,
            candidates.find((item) => item.poiid === event.target.value),
          )
        }
      >
...
function PoiMatchDiagnostic({
  ...
  return (
    <div className="row-diagnostic">
      <button className="small ghost" onClick={() => void copyDiagnostic()}>
        {copied ? '已复制 ✓' : '复制记录'}
      </button>
      <pre>{diagnostic}</pre>
    </div>
